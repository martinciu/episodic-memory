 ...
 */
export function migrateVecDimension(db: Database.Database): void {
  const row = db.prepare(
    `SELECT sql FROM sqlite_master WHERE type='table' AND name='vec_exchanges'`
  ).get() as { sql: string } | undefined;
  if (!row) return; // fresh database — initDatabase creates the table at the current dim
  if (row.sql.includes(`FLOAT[${EMBEDDING_DIM}]`)) return; // already current
...
export function initDatabase(): Database.Database {
  ...
  // Create vector search index. If an existing table was built for an older
  // encoder's dimension, drop it first so it is recreated at the current one.
