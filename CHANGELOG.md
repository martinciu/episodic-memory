...
## [1.4.2-martinciu.3] - 2026-08-13 (fork: martinciu/episodic-memory)

### Added (ported from danielsimonjr/episodic-memory — same lineage, authorship preserved)
- **`index prune`** removes already-indexed projects from the database — `--excluded` prunes every project on the exclude list, `--project <name>` prunes one, `--dry-run` reports row counts and reclaimable text bytes without writing. Until now, `exclude.txt` only stopped *future* indexing; rows already indexed stayed searchable forever, and removing them meant hand-written SQL that silently orphans vector rows when run without the sqlite-vec extension loaded (addresses upstream #141).
- **`index vacuum`** shrinks the database file. SQLite never returns freed pages to the filesystem on its own, so deletions reclaimed no disk space — a 3 GB database stayed 3 GB no matter what was deleted. Reports reclaimable pages before running and needs temporary free space roughly equal to the database size (addresses upstream #140).
- **Oversized messages are capped at index time.** Third-party summarizer agents embed whole conversations in their prompts; indexed verbatim, a single "user message" could be megabytes (97.2% of one real 3 GB database). Messages are now capped at 256 KB with a truncation notice recording the original length; override with `EPISODIC_MEMORY_MAX_MESSAGE_BYTES`, `0` disables (addresses upstream #139).
